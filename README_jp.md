...
| [`scarpet-fmt`](scarpet-fmt) | コードフォーマッタ。CST を Wadler/Lindig 流のプリティプリント用 IR に変換し、固定スタイルで描画します。 |
...
終了コード: `0` 成功、`1` 解析エラーまたは `--check` 失敗、`2` 入出力エラー。

### 整形スタイル

MVP のスタイルは固定（インデント 4 スペース、目標 100 桁）で、まだ設定変更はできません。主な点:
