...
comment_width = -1        # コメントの行幅の目標。-1 で折り返しを無効化
line_ending = "lf"        # 改行コード: "lf"（デフォルト）, "crlf", "auto"（ソースに合わせる）, "native"（実行環境）
brace_style = "same_line" # 折り返したブロックの開き区切り: "same_line"（デフォルト）または "next_line"
```

未知のキー、`max_width = 0`、`comment_width = 0` または `-1` 未満、`"lf"`・`"crlf"`・`"auto"`・`"native"` 以外の `line_ending`、および `"same_line"`・`"next_line"` 以外の `brace_style` は拒否されます。これらの設定項目を除けばレイアウトは固定です。主な点:
...
- コメントは保持されます。独立行のコメントは独立行のまま、行末コメントは付いていた行のまま維持します。`comment_width` が正の値の場合、長い `//` コメントをその幅で折り返します。`-1` では折り返しません。連続する空行は最大 `blank_lines_upper_bound`（デフォルト 1）個までにまとめます。
...
`brace_style = "next_line"` を指定すると、折り返した呼び出しや関数本体の開き区切りが代わりに独立した行から始まります（Allman スタイル）。1 行に収まるブロックは影響を受けません。
