...
- `scarpet-fmt` — formatter: lowers the CST to a Wadler/Lindig pretty-printing `Doc` IR, then renders it at a fixed style.
...
```sh
cargo run -p scarpet-cli -- format <file>         # format to stdout (also: --in-place, --check, stdin)
