...
| File | Responsibility |
| --- | --- |
| `config.rs` | The `Config` style struct (`indent_width`, `max_width`, `comment_width`, `line_ending`, `brace_style`) and its `Default`. |
...
| `binop_separator` | 🟡 | operator wrap position; currently `Back`-only (rustfmt defaults `Front`) |
